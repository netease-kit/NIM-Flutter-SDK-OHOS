...
class FriendService {
  ...
  @loggerDec
    async clearAllAddApplication(): Promise<NIMResult<void>> {
      try {
        return successRes(
          await this.nim.V2NIMFriendService.clearAllAddApplication()
        )
      } catch (error) {
        throw failRes(error as V2NIMError)
      }
   ...
   * 监听添加好友
   */
  private async _onFriendAdded(friend: V2NIMFriend) {
    logger.log('_onFriendAdded', friend)
    if (friend.userProfile === undefined || friend.userProfile === null) {
      try {
        var users = await this.nim.V2NIMUserService.getUserList([friend.accountId])
        if (users.length > 0) {
          friend.userProfile = users[0]
        }
      } catch (error) {
        logger.log('_onFriendAdded', error)
      }
   ...
   * 好友信息更新回调，返回变更的好友信息，包括本端直接更新的好友信息和其他端同步更新的好友信息
   */
  private async _onFriendInfoChanged(friend: V2NIMFriend) {
    ...
    if (friend.userProfile === undefined || friend.userProfile === null) {
      try {
        ...
        logger.log('_onFriendInfoChanged', error)
      }
