...
extension NIMSDKOption {
  ...
  /// - Returns: 对象
  static func fromDic(_ arguments: [String: Any]) -> NIMSDKOption {
    let option = NIMSDKOption()

    if let appKey = arguments["appKey"] as? String {
      option.appKey = appKey
    }

    if let apnsCername = arguments["apnsCername"] as? String {
      option.apnsCername = apnsCername
    }

    if let pkCername = arguments["pkCername"] as? String {
      option.pkCername = pkCername
    }

    if let avoidNosAccelerationBuckets = arguments["avoidNosAccelerationBuckets"] as? [String] {
      let avoidNosAccelerationBuckets = Set(avoidNosAccelerationBuckets)
      option.avoidNosAccelerationBuckets = avoidNosAccelerationBuckets
    }

    if let v2 = arguments["v2"] as? Bool {
      option.v2 = v2
    }
  ...
  func toDic() -> [String: Any] {
    ...
    keyPaths[#keyPath(appKey)] = appKey
    keyPaths[#keyPath(apnsCername)] = apnsCername
    keyPaths[#keyPath(pkCername)] = pkCername
    keyPaths[#keyPath(avoidNosAccelerationBuckets)] = avoidNosAccelerationBuckets
    keyPaths[#keyPath(v2)] = v2
    return keyPaths
  }
}
